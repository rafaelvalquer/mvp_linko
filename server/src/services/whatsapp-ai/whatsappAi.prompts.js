...
function buildIntentRoutingInstructions() {
  return [
    ...
    "Leia mensagens em pt-BR.",
    "Responda somente com JSON valido e sem markdown.",
    "Classifique se a mensagem e sobre criar proposta, revisar propostas enviadas hoje, consultar status de uma proposta, revisar propostas expirando, reenviar proposta, consultar agenda diaria, consultar agenda semanal, consultar proximo compromisso, reagendar compromisso, cancelar agendamento, consultar propostas pendentes, consultar cobrancas de hoje, consultar cobrancas atrasadas, retomar follow-up de cobranca, revisar prioridades de cobranca do dia, listar propostas aguardando confirmacao para aprovar comprovante, cobrar cliente de uma proposta, cancelar proposta, criar automacao, listar automacoes, pausar automacao, retomar automacao, executar automacao agora, duplicar automacao, excluir automacao, cadastrar cliente, cadastrar produto, atualizar preco de produto, consultar telefone de cliente ou consultar produtos, ou se esta ambigua.",
    "Retorne ambiguous_proposal_operation quando houver duvida real entre criar proposta, revisar propostas de hoje, consultar status, ver expirando ou reenviar proposta.",
    ...
    "Retorne query_recent_offers quando a mensagem pedir propostas enviadas hoje, propostas criadas hoje ou propostas do dia.",
    "Retorne query_offer_status quando a mensagem pedir o status de uma proposta, andamento da proposta ou situacao da proposta.",
    "Retorne query_expiring_offers quando a mensagem pedir propostas expirando, propostas que vencem hoje ou propostas que vencem nesta semana.",
    "Retorne resend_offer_link quando a mensagem pedir para reenviar uma proposta, reenviar o link da proposta ou mandar a proposta novamente.",
    "Retorne query_pending_offers quando a mensagem pedir propostas pendentes, lista de pendentes ou cobrancas pendentes.",
    "Retorne query_due_today_offers quando a mensagem pedir quem precisa ser cobrado hoje ou cobrancas que vencem hoje.",
    "Retorne query_overdue_offers quando a mensagem pedir cobrancas atrasadas, vencidas ou em atraso.",
    "Retorne query_stale_offer_followups quando a mensagem pedir propostas sem resposta, follow-ups parados ou retomada de cobranca.",
    "Retorne query_billing_priorities quando a mensagem pedir prioridades de cobranca do dia ou um resumo do que atacar hoje.",
    ...
    "Retorne automation_create quando a mensagem pedir para criar, configurar ou agendar uma automacao pessoal.",
    "Retorne automation_list quando a mensagem pedir para ver, listar ou consultar automacoes ja cadastradas.",
    "Retorne automation_pause quando a mensagem pedir para pausar, parar ou suspender uma automacao.",
    "Retorne automation_resume quando a mensagem pedir para retomar, reativar ou ativar novamente uma automacao.",
    "Retorne automation_run_now quando a mensagem pedir para executar uma automacao agora, rodar agora ou disparar imediatamente.",
    "Retorne automation_duplicate quando a mensagem pedir para duplicar ou copiar uma automacao.",
    "Retorne automation_delete quando a mensagem pedir para excluir, apagar ou remover uma automacao.",
    "Retorne unknown quando a mensagem nao estiver relacionada aos fluxos de proposta, agenda, cobranca e vendas, automacoes ou backoffice.",
    "source_text deve refletir a compreensao final do texto informado.",
  ].join("\n");
}
...
function buildOfferSalesOperationInstructions() {
  return [
    ...
    "Leia mensagens em pt-BR.",
    "Responda somente com JSON valido e sem markdown.",
    "Extraia apenas parametros operacionais para revisar propostas enviadas hoje, consultar status de uma proposta, revisar propostas expirando, reenviar proposta, consultar propostas pendentes, listar propostas aguardando confirmacao para aprovar comprovante, cobrar cliente de uma proposta ou cancelar proposta.",
    "Use intent=query_recent_offers quando a mensagem pedir propostas enviadas hoje, propostas criadas hoje ou propostas do dia.",
    "Use intent=query_offer_status quando a mensagem pedir o status, andamento ou situacao de uma proposta.",
    "Use intent=query_expiring_offers quando a mensagem pedir propostas expirando, propostas que vencem hoje ou propostas desta semana.",
    "Use intent=resend_offer_link quando a mensagem pedir para reenviar a proposta ou mandar o link da proposta novamente.",
    "Use intent=query_pending_offers quando a mensagem pedir propostas pendentes ou cobrancas pendentes.",
    ...
    "Use intent=send_offer_payment_reminder quando a mensagem pedir para cobrar um cliente, enviar lembrete ou cobrar uma proposta especifica.",
    "Use intent=cancel_offer quando a mensagem pedir para cancelar uma proposta.",
    "target_customer_name deve ser preenchido quando o usuario mencionar o cliente ou um titulo/nome que ajude a localizar a proposta.",
    ...
    "Nao invente cliente, titulo ou data ausentes.",
    "source_text deve refletir a compreensao final do texto informado.",
  ].join("\n");
}
