...
function resolveSchedulerLeaseTtlMs(intervalMs) {
  return Math.max(
    30_000,
    Number(
      process.env.PAYMENT_REMINDERS_RUNNER_SCHEDULER_LOCK_TTL_MS ||
        intervalMs * 2,
    ),
  );
}
...
export async function runPaymentRemindersCycle(options = {}) {
  ...
  try {
    const summary = await runWithDistributedLease(
      ...
      async (lease) => {
        console.log("[payment-reminders-runner]", {
          role: env.appRole,
          runner: RUNTIME_NAME,
          lockId: lease?.lockId || null,
          reason: "cycle_started",
        });
        return processAutomaticPaymentReminders(options);
      },
    );
...
export function stopPaymentRemindersRunner() {
